...
export default function ManagerOrdersPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    const filters: ManagerOrderFilters = {
      scope: scope as ManagerOrderFilters['scope'],
      page,
      size: PAGE_SIZE,
    };
  ...
  const applyStatus = async (order: Order) => {
    ...
    if (!target) return;
    if (target === 'CANCELLED') {
      // отмена необратима — требуем подтверждения (plan-02)
      setCancelling(order);
      return;
    }
    setBusy(order.id);
  ...
  const confirmCancel = async () => {
    ...
    try {
      ...
      push(`Заказ ${order.number} → ${orderStatusLabel('CANCELLED')}`, 'success');
      setCancelling(null);
      load();
    } catch (err) {
      push((err as Error).message, 'error');
  ...
  const columns = useMemo(() => {
    const base = [
      ...
      },
      {
        key: 'status',
        title: 'Статус',
        render: (o: Order) => orderStatusBadge(o.status as OrderStatus),
      },
      {
        key: 'recipient',
        title: 'Получатель',
      ...
      render: (o: Order) => {
        ...
        return (
          <div className={styles.rowActions}>
            ...
            <Button
              loading={busy === o.id}
              disabled={!nextStatus[o.id]}
              onClick={() => applyStatus(o)}
            >
              Применить
            </Button>
          </div>
  ...
  return (
    <div className={styles.page}>
      ...
      >
        <p>
          Отменить заказ {cancelling?.number} на сумму{' '}
          {cancelling ? formatMoney(cancelling.totalAmount) : ''}? «Отменён» — финальный статус,
